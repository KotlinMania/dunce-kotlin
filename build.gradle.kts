...
kotlin {
    ...
    swiftExport {
        moduleName = "Dunce"
        flattenPackage = "io.github.kotlinmania.dunce"
    }

    android {
        namespace = "io.github.kotlinmania.dunce"
        compileSdk = 34
        minSdk = 24
...
mavenPublishing {
    ...
    coordinates(group.toString(), "dunce-kotlin", version.toString())

    pom {
        name.set("dunce-kotlin")
        description.set("Kotlin Multiplatform port of the Rust crate `dunce` - Normalize Windows paths to the most compatible format, avoiding UNC where possible")
        inceptionYear.set("2026")
        url.set("https://github.com/KotlinMania/dunce-kotlin")
...
val codeqlCompileJvm = tasks.register<JavaExec>("codeqlCompileJvm") {
    ...
    val platformSources = fileTree("src/jvmMain/kotlin") {
        include("**/*.kt")
    }
    ...
    doFirst {
        ...
        if (sourceFiles.isEmpty()) {
            val sentinelFile = sentinelDir.get().asFile.resolve("io/github/kotlinmania/dunce/codeql/_CodeqlEmptySource.kt")
...
val fullTargetBuildTaskNames = setOf(
    ...
    "assembleDunceXCFramework",
)

afterEvaluate {
    tasks.named("build") {
        dependsOn(fullTargetBuildTaskNames.mapNotNull { taskName -> tasks.findByName(taskName) })
        dependsOn(
            tasks.matching {
