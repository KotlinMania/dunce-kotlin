# Claude Code Project Instructions — dunce-kotlin

## Project Overview

This is **dunce-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`dunce`](https://crates.io/crates/dunce).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/dunce/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/dunce>
